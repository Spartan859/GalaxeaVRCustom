...
class R1ProConfig:
    ...
    floating_base_pose_topic: str = "/motion_control/pose_floating_base"
    ...
    obs_floating_base: str = "floating_base_pose"
...
class _R1Node(Node):
    def __init__(self, cfg: R1ProConfig):
        ...
        self._floating_base_pose: Optional[np.ndarray] = None
        ...
        # floating base pose from motion_control
        self.create_subscription(
            PoseStamped,
            cfg.floating_base_pose_topic if hasattr(cfg, 'floating_base_pose_topic') else "/motion_control/pose_floating_base",
            self._on_floating_base_pose,
            qos,
        )
    ...
    def _on_left_ee_pose(self, msg: PoseStamped):
        with self._lock:
            ...
            # self.get_logger().info(f"left ee pose update: t={msg.header.stamp.sec}.{msg.header.stamp.nanosec} x={p.x:.3f} y={p.y:.3f} z={p.z:.3f}")
    ...
    def _on_floating_base_pose(self, msg: PoseStamped):
        with self._lock:
            p = msg.pose.position
            q = msg.pose.orientation
            self._floating_base_pose = np.array([p.x, p.y, p.z, q.x, q.y, q.z, q.w], dtype=np.float32)
    ...
    #     # ===== 保存路径 =====
    #     save_dir = "/home/nvidia/gwx/data"
    #     os.makedirs(save_dir, exist_ok=True)

    #     ts = time.time()
    #     filename = f"{key}_{ts:.3f}.jpg"
    ...
    #         self._img_count = {}
    #     self._img_count[key] = self._img_count.get(key, 0) + 1
    #     if self._img_count[key] <= 3:
    #         self.get_logger().info(f"[CAM] saved {path}")
    ...
    def publish(
        ...
        # print("Publishing poses/grippers")
    ...
    def publish_joints(
        ...
        # print("Publishing joints/grippers")
        stamp = self.get_clock().now().to_msg()
    ...
    def publish_joint_reset_targets(
        ...
        # print("Publishing joint reset targets")
        stamp = self.get_clock().now().to_msg()
...
class R1ProRobot:
    ...
    def __init__(self, cfg: R1ProConfig):
        ...
        # background spinner to ensure subscriptions get serviced
    ...
    def connect(self):
        ...
        # spin subscriptions in background
    ...
    def _spin_loop(self):
        """Continuously spin to service subscriptions in the background."""
        while self.is_connected and not self._spin_stop.is_set():
            try:
                self.spin_once(0.05)
            except Exception:
                # keep spinning even if one iteration fails
                time.sleep(0.1)
    ...
    def get_observation(self) -> Optional[Dict[str, Any]]:
        ...
        if not self._node:
            return None
        l_js, r_js, l_g, r_g, images, l_pose, r_pose, base_pose = self._node.get_state()

        if self.cfg.use_joints:
            if None in (l_js, r_js, l_g, r_g):
                return None
        else:
            if any(x is None for x in (l_js, r_js, l_g, r_g, l_pose, r_pose, base_pose)):
                return None
        ...
        if self.cfg.use_joints:
            ...
            obs = {
                ...
                self.cfg.obs_left_gripper: np.array([l_g], dtype=np.float32),
                self.cfg.obs_right_gripper: np.array([r_g], dtype=np.float32),
                self.cfg.obs_floating_base: base_pose,
            }
        if self.cfg.enable_cameras:
    ...
    def _ensure_target_initialized(self):
        ...
        # 初始化时也做一次 reset（对齐 open_box 的 command 2）
    ...
    def reset(self):
        """执行一次关节 reset：对齐 open_box 的 command 2，并在期间阻塞 action；完成后刷新观测并发布初始化位姿。"""
        ...
        # 同步将双侧夹爪打开到最大
        try:
            self._node.publish(None, None, self._cfg.gripper_max if hasattr(self, '_cfg') else self.cfg.gripper_max, self._cfg.gripper_max if hasattr(self, '_cfg') else self.cfg.gripper_max)
        except Exception:
            pass
        ...
        # 等待 reset 完成一段时间，并更新一次观测，然后设置并发布初始化位姿
        # 等待观测稳定：只要观测还在变化就持续等待，无超时
        obs: Optional[Dict[str, Any]] = None
        last_obs: Optional[Dict[str, Any]] = None
        stable_count = 0

        def _obs_close(o1: Dict[str, Any], o2: Dict[str, Any]) -> bool:
            keys = [
                self.cfg.obs_left_ee,
                self.cfg.obs_right_ee,
                self.cfg.obs_left_gripper,
                self.cfg.obs_right_gripper,
                self.cfg.obs_floating_base,
            ]
            for k in keys:
                ...
                if isinstance(v1, np.ndarray) or isinstance(v2, np.ndarray):
                    if not np.allclose(np.array(v1), np.array(v2), atol=1e-4, rtol=1e-4):
                        return False
                else:
        ...
        while self.is_connected:
            # background spin thread already services callbacks; just pause briefly
            time.sleep(0.05)
        ...
        # reset 后夹爪目标强制设为最大打开
        self._target_left_grip = float(self.cfg.gripper_max)
        self._target_right_grip = float(self.cfg.gripper_max)
    ...
    def send_action(self, action: Dict[str, Any]):
        """
        接收 action：
            - "left_ee_pose":  [dx,dy,dz,droll,dpitch,dyaw]  (全部为增量，弧度)
            - "right_ee_pose": [dx,dy,dz,droll,dpitch,dyaw]
            - "left_gripper":  float 或 [float] （delta张开度，mm）
            - "right_gripper": float 或 [float]
        ...
        姿态增量：
            q_delta = euler_to_quat(droll, dpitch, dyaw)
            q_new   = q_curr ⊗ q_delta
        """
        # 支持在 send_action 中触发 reset
        if bool(action.get("reset", False)):
            self.reset()
            return
        # 在 reset 期间不接受 action
        if self._is_resetting:
            raise RuntimeError("reset in progress; action rejected")

        # 确保 target_* 已用当前观测初始化（包含当前姿态四元数）
        ...
        if l_cmd is not None:
            ...
            if l_cmd.shape[0] < 6:
                raise ValueError(f"left_ee_pose must be length >= 6, got {l_cmd.shape}")

            dx, dy, dz, droll, dpitch, dyaw = l_cmd[:6]

            # 位置：增量累加
            self._target_left_pose[0:3] += np.array([dx, dy, dz], dtype=np.float32)

            # 姿态：用增量四元数右乘当前四元数
            q_curr = self._target_left_pose[3:7]
            q_delta = self._euler_to_quat(droll, dpitch, dyaw)  # 小转动
            q_new = self._quat_multiply(q_curr, q_delta)        # q_new = q_curr ⊗ q_delta
            self._target_left_pose[3:7] = q_new

        # ---------- 右臂 ----------
        if r_cmd is not None:
            r_cmd = np.array(r_cmd, dtype=np.float32).reshape(-1)
            if r_cmd.shape[0] < 6:
                raise ValueError(f"right_ee_pose must be length >= 6, got {r_cmd.shape}")

            dx, dy, dz, droll, dpitch, dyaw = r_cmd[:6]

            # 位置：增量累加
            self._target_right_pose[0:3] += np.array([dx, dy, dz], dtype=np.float32)

            # 姿态：用增量四元数右乘当前四元数
            q_curr = self._target_right_pose[3:7]
            q_delta = self._euler_to_quat(droll, dpitch, dyaw)
            q_new = self._quat_multiply(q_curr, q_delta)
            self._target_right_pose[3:7] = q_new

        # ---- gripper 处理（delta 累积 target）----
        lg = action.get(self.cfg.act_left_gripper)
        rg = action.get(self.cfg.act_right_gripper)

        # 解析 delta（允许 float 或 [float]）
        dl = None
        dr = None
        if lg is not None:
            dl = float(lg[0]) if isinstance(lg, (list, np.ndarray)) else float(lg)
        if rg is not None:
            dr = float(rg[0]) if isinstance(rg, (list, np.ndarray)) else float(rg)

        # 如果有 delta，累积到 target

        self._target_left_grip += dl
        self._target_right_grip += dr

        # 限幅
        self._target_left_grip = float(
            np.clip(self._target_left_grip, self.cfg.gripper_min, self.cfg.gripper_max)
        )
        self._target_right_grip = float(
            np.clip(self._target_right_grip, self.cfg.gripper_min, self.cfg.gripper_max)
        )
    ...
    def wait_ready(self) -> bool:
        ...
        while time.time() < deadline and self.is_connected:
            # background spin thread handles callbacks; sleep briefly while waiting
            time.sleep(0.05)
            obs = self.get_observation()
            if obs is not None:
                # 初始化一次 target
            ...
            try:
                ...
                if cmd == "wait_ready":
                    ...
                    while time.time() < deadline and self.robot.is_connected:
                        # callbacks are handled by background spin thread; sleep briefly
                        time.sleep(0.05)
...
def main():
    cfg = R1ProConfig(
        enable_cameras=True,  # 先只调通控制；需要图像时改为 True
        use_joints=False,
    )
